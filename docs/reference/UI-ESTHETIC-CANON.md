...
- garder le meme langage esthetique que `kyclink-node`
...
Note: `/complete` a ete supprime sans redirection (unification de l'ecran resultat, 2026-07-25). Le seul ecran de resultat est desormais `SESSIONS/:sessionId`.
...
- les ecrans `SESSIONS/:sessionId` et `SESSIONS` exposent seulement le statut, la reference utile, la decision observable et les actions de suite
...
- `SESSION_CONTEXT`, `SESSION_PREPARE`, `SESSIONS/:sessionId` et `SESSIONS` utilisent une icone retour a destination canonique par etape, sans dependre de l'historique navigateur
- l'icone retour de `SESSIONS/:sessionId` renvoie vers `SESSIONS`
...
- les labels editoriaux de type `WELCOME`, `SESSIONS`, `SESSIONS/:sessionId` ou `KYC_LINK` ne doivent pas etre affiches comme titres d'ecran
...
- `SESSIONS` et `SESSIONS/:sessionId` peuvent afficher un message inline seulement pour une indisponibilite restant sur l'ecran, jamais pour une expiration de session Cognito
...
- hero centre puis carte `surface-light`, dans le meme vocabulaire que kyclink-node
...
- `SESSIONS/:sessionId` et `SESSIONS` gardent des actions courtes et des resumees lisibles, sans sur-explication
- `WELCOME`, `SESSIONS` et `SESSIONS/:sessionId` reutilisent le meme contrat: header fixe, body interne scrollable, actions separees du contenu principal
...
- `SESSIONS/:sessionId` privilegie aussi des actions utilitaires en icones avec `title`/`aria-label`, avec sortie positive explicite vers l'accueil quand disponible
...
- un composant repris de kyclink-node doit etre copie puis maintenu localement
- `PageShell`, `SurfacePanel` et les ecrans proteges reprennent le meme frame mobile que kyclink-node
...
- ne pas importer de CSS, de composants ou de tokens depuis kyclink-node
...
- les layouts de parcours gardent la meme logique de guidage que kyclink-node
- aucune dependance cross-project n'est introduite pour les styles ou composants
- les ecrans principaux restent visuellement dans la meme famille que kyclink-node
